...
void ChatMessage::to_bin()
{
    ...
    aux += sizeof(uint8_t);
    memcpy(aux, nick.c_str(), sizeof(char)* 10);
    aux += sizeof(char) * 10;
...
int ChatMessage::from_bin(char * bobj)
{
    ...
    //Reconstruir la clase usando el buffer _data
     ...
     aux += sizeof(uint8_t);
     char nickkk[10] = {0};
     memcpy(nickkk, aux, 10* sizeof(char));
     nick = std::string(nickkk);
     aux += 10* sizeof(char);
     char message[81] = {0};
     memcpy(message, aux, 80 * sizeof(char));
     message = std::string(message); return 0;
